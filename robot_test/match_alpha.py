...
alpha = matching.match(np.array(re,dtype=np.uint8),'D:/wurenji/summerSchool-fan_v_rep_branch/alpha_img')
print(alpha)
state=matching.match_alpha(np.array(re,dtype=np.uint8))
# f = open('00.txt', 'w')
# f.write(str(re))
# f.close()
print(state[0][0][0])
