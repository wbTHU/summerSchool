...
def camShift(nowImg, preImg, preWin):

    x,y,w,h = preWin
    r = preImg[y:y+h, x:x+w]
    # cv2.imshow('pre',r)
    # cv2.waitKey()
    ...
    ret, nowWin = cv2.CamShift(dst, preWin, term_crit)

    # print(nowWin)

    # x,y,w,h = nowWin
    # r = nowImg[y:y+h, x:x+w]
    # cv2.imshow('now',r)
    # cv2.waitKey()
...
now = cv2.imread('1.png')

t = camShift(now,frame, track_window)
